...
void main() {
    ...
    vec2 absoluteDerivatives = fwidth(vUv) * 2.0;
    gl_FragColor = vColor * (rounded(absolute, vOuterSize, absoluteDerivatives) - rounded(absolute * vOuterToInner, vInnerSize, absoluteDerivatives * vOuterToInner));
    if (u_useMask) {
