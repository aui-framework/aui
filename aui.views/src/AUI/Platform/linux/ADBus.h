...
struct basic_converter {
    ...
    static void iter_append(DBusMessageIter* iter, const T& t) { dbus_message_iter_append_basic(iter, dbusType, &t); }
...
template <>
struct converter<std::nullopt_t> {   // -> DBUS_TYPE_INVALID
...
template <>
struct converter<std::uint8_t> : impl::basic_converter<std::uint8_t, DBUS_TYPE_BYTE> {};
template <>
struct converter<std::int16_t> : impl::basic_converter<std::int16_t, DBUS_TYPE_INT16> {};
template <>
struct converter<std::uint16_t> : impl::basic_converter<std::uint16_t, DBUS_TYPE_UINT16> {};
template <>
struct converter<std::int32_t> : impl::basic_converter<std::int32_t, DBUS_TYPE_INT32> {};
template <>
struct converter<std::uint32_t> : impl::basic_converter<std::uint32_t, DBUS_TYPE_UINT32> {};
template <>
struct converter<std::int64_t> : impl::basic_converter<std::int64_t, DBUS_TYPE_INT64> {};
template <>
struct converter<std::uint64_t> : impl::basic_converter<std::uint64_t, DBUS_TYPE_UINT64> {};
template <>
struct converter<double> : impl::basic_converter<double, DBUS_TYPE_DOUBLE> {};
template <>
struct converter<const char*> : impl::basic_converter<const char*, DBUS_TYPE_STRING> {};
template <>
...
template <>
struct converter<std::string> : converter<const char*> {
    static void iter_append(DBusMessageIter* iter, const std::string& t) {
        converter<const char*>::iter_append(iter, t.c_str());
    }
    static std::string iter_get(DBusMessageIter* iter) { return converter<const char*>::iter_get(iter); }
};

template <>
struct converter<AString> : converter<std::string> {
    static void iter_append(DBusMessageIter* iter, const AString& t) {
        converter<std::string>::iter_append(iter, t.toStdString());
    }
    static AString iter_get(DBusMessageIter* iter) { return converter<const char*>::iter_get(iter); }
};
template <>
struct converter<ObjectPath> : impl::basic_converter<const char*, DBUS_TYPE_OBJECT_PATH> {
    using super = impl::basic_converter<const char*, DBUS_TYPE_OBJECT_PATH>;
    static void iter_append(DBusMessageIter* iter, const ObjectPath& t) { super::iter_append(iter, t.c_str()); }

    static ObjectPath iter_get(DBusMessageIter* iter) { return super::iter_get(iter); }
};
template <std::size_t N>
struct converter<char[N]> : converter<const char*> {};
...
struct converter<AVector<T>> {
    ...
    static void iter_append(DBusMessageIter* iter, const AVector<T>& t) {
        ...
        ARaiiHelper h = [&] { dbus_message_iter_close_container(iter, &sub); };
...
struct converter<AMap<K, V>> {
    ...
    static void iter_append(DBusMessageIter* iter, const AMap<K, V>& t) {
        ...
        if (!dbus_message_iter_open_container(
                ...
                fmt::format("{{{}{}}}", converter<K>::signature, converter<V>::signature).c_str(), &sub)) {
            throw AException("dbus_message_iter_open_container failed");
        }
        ARaiiHelper h = [&] { dbus_message_iter_close_container(iter, &sub); };
        ...
        for (const auto& [k, v] : t) {
            ...
            ARaiiHelper r = [&] { dbus_message_iter_close_container(&sub, &item); };
...
struct converter<std::tuple<Types...>> {
    ...
    static void iter_append(DBusMessageIter* iter, const std::tuple<Types...>& t) {
        ...
        std::apply([&](const auto&... args) { (..., aui::dbus::iter_append(&sub, args)); }, t);
    }
...
public:
    ...
    static ADBus& session();

    template <aui::dbus::convertible... Args>
    void call(
        const AString& bus, const AString& path, const AString& interface, const AString& method, const Args&... args) {
        auto msg = aui::ptr::manage_shared(
            dbus_message_new_method_call(
                bus.toStdString().c_str(), path.toStdString().c_str(), interface.toStdString().c_str(),
...

        {
            ...
            aui::parameter_pack::for_each([&](const auto& v) { aui::dbus::iter_append(&dbusArgs, v); }, args...);
        }
        UnixIoThread::inst().thread()->enqueue(
            [this, msg = std::move(msg), formatError = std::move(formatError)]() mutable {
                if (!dbus_connection_send(mConnection, msg.get(), nullptr)) {
                    ALogger::err("ADBus::call") << formatError("dbus_connection_send failed");
                }
            });
    }

    template <aui::dbus::convertible_or_void Return, aui::dbus::convertible... Args>
    AFuture<Return> callWithResult(
        const AString& bus, const AString& path, const AString& interface, const AString& method, const Args&... args) {
        auto msg = aui::ptr::manage_shared(
            dbus_message_new_method_call(
                bus.toStdString().c_str(), path.toStdString().c_str(), interface.toStdString().c_str(),
        ...
        AFuture<Return> result;
        UnixIoThread::inst().thread()->enqueue(
            [this, msg = std::move(msg), result, formatError = std::move(formatError)]() mutable {
              try {
                  auto pending = aui::ptr::manage_unique(
                      [&] {
                        DBusPendingCall* pending;
                        if (!dbus_connection_send_with_reply(mConnection, msg.get(), &pending, -1)) {   // -1 is
                            // default
                            // timeout
                  ...
                  auto handler = [result = std::move(result), formatError = std::move(formatError)](DBusPendingCall* pending) {
                    try {
                        auto msg =
                            aui::ptr::manage_unique(dbus_pending_call_steal_reply(pending), dbus_message_unref);
                        ...
                        if constexpr (!std::is_void_v<Return>) {
                            ...
                            result.supplyValue(aui::dbus::iter_get<Return>(&dbusArgs));
                        } else {
                            result.supplyValue();
                        }
                    } catch (...) {
                  ...
                  dbus_pending_call_set_notify(
                      ...
                      [](DBusPendingCall* pending, void* data) {
                        auto& result = *static_cast<decltype(handler)*>(data);
                        result(pending);
                      },
                      new decltype(handler)(std::move(handler)),
                      [](void* data) {
                        auto result = static_cast<decltype(handler)*>(data);
                        delete result;
                      });
    ...
    struct MessageFilter {
        using Ownership = std::unique_ptr<MessageFilter, void(*)(MessageFilter*)>;
        ADBus* parent;
    ...
    MessageFilter::Ownership addSignalListener(
        aui::dbus::ObjectPath object, const AString& interface, const AString& signal, Callback&& callback) {
        return addFilter(
            [object = std::move(object), interface = interface.toStdString(), signal = signal.toStdString(),
    ...
    DBusConnection* handle() const noexcept { return mConnection; }

    void processMessages();

    std::string_view uniqueName() const noexcept;


    MessageFilter::Ownership addFilter(MessageFilter::Callback listener);

private:
    std::list<MessageFilter> mListeners;   // guarantees safe pointers to it's elements
    ...
    static DBusHandlerResult listener(DBusConnection* connection, DBusMessage* message, void* user_data) noexcept;
