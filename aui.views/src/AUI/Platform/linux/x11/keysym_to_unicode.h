 ...
 * Follows the X.Org / Unicode Consortium / Markus Kuhn KeySym to Unicode mapping specification.
 */
inline char32_t keysymToUnicode(KeySym keysym) noexcept {
    // Exclude Unicode surrogates (0x0100d800 - 0x0100dfff)
    if (keysym >= 0x0100d800 && keysym <= 0x0100dfff) {
        return 0;
    }
    ...
    // Special ASCII / keypad keysyms (BackSpace, Tab, Return, Escape, KP_*, Delete, etc.)
    if ((keysym >= 0xff08 && keysym <= 0xff0b) ||
        ...
        return static_cast<char32_t>(keysym & 0x7f);
    }
    ...
    static constexpr Codepair keysymtab[] = {
        { 0x01a1, 0x0104 },
        { 0x01a2, 0x02d8 },
        { 0x01a3, 0x0141 },
        { 0x01a5, 0x013d },
    ...
    auto it = std::lower_bound(
        ...
        [](const Codepair& p, KeySym k) { return p.keysym < k; });
    if (it != std::end(keysymtab) && it->keysym == keysym) {
        return it->ucs;
    }
