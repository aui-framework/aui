...
namespace RenderHints {
    ...
    public:
        ...
        CanvasOrRenderer(IRendererBackend& backend) : mVal(&backend) {}

        glm::mat4 getTransform() const {
            if (auto* r = std::get_if<IRenderer*>(&mVal)) return (*r)->getTransform();
            if (auto* b = std::get_if<IRendererBackend*>(&mVal)) return (*b)->getTransform();
        ...
        void setTransformForced(const glm::mat4& t) {
            ...
            else if (auto* b = std::get_if<IRendererBackend*>(&mVal)) (*b)->setTransformForced(t);
            else std::get<ACanvas*>(mVal)->setTransformForced(t);
        }
        AColor getColor() const {
            if (auto* r = std::get_if<IRenderer*>(&mVal)) return (*r)->getColor();
            if (auto* b = std::get_if<IRendererBackend*>(&mVal)) return (*b)->getColor();
