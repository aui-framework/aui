        ...
        Horizontal {
            Vertical {
                _container<AAdvancedGridLayout>({
                    ...
                    wrapper AUI_OVERRIDE_STYLE {AOverflow::HIDDEN },
                }, 2, 2) AUI_OVERRIDE_STYLE { Expanding { 2, 2 } },
                _new<AScrollbar>(ALayoutDirection::HORIZONTAL),

            } AUI_OVERRIDE_STYLE { Expanding { 2, 2 } },
            _new<AScrollbar>(),
        }
    );
...
void ARulerArea::onLayout(glm::ivec2 size) {
    mWrappedView->setSize(mWrappedView->measure(AConstraints {}));
    updatePosition();
