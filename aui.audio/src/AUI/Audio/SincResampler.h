...
struct RepeatingCallback {
  using FnType = std::function<Signature>;
  FnType fn;
  RepeatingCallback(FnType&& fn_in) : fn(std::move(fn_in)) {}
...
public:
  ...
  constexpr explicit raw_ptr(T* ptr) noexcept : ptr_(ptr) {}

  constexpr raw_ptr(const raw_ptr& other) noexcept : ptr_(other.ptr_) {}

  auto operator=(T* ptr) noexcept -> raw_ptr& {
    ptr_ = ptr;
    return *this;
  }

  auto operator=(const raw_ptr& other) noexcept -> raw_ptr& {
    ptr_ = other.ptr_;
    return *this;
  }
  ...
  constexpr auto operator==(const raw_ptr& other) const noexcept -> bool { return ptr_ == other.ptr_; }
  constexpr auto operator!=(const raw_ptr& other) const noexcept -> bool { return ptr_ != other.ptr_; }

  template <typename U = Traits>
  constexpr typename std::enable_if<std::is_same<U, AllowPtrArithmetic>::value, raw_ptr>::type
  operator+(std::ptrdiff_t offset) const noexcept {
    return raw_ptr(ptr_ + offset);
  }
  ...
  template <typename U = Traits>
  constexpr typename std::enable_if<std::is_same<U, AllowPtrArithmetic>::value, std::ptrdiff_t>::type
  operator-(const raw_ptr& other) const noexcept {
