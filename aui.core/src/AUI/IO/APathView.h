...
/**
 * @brief An add-on to AStringView with functions for working with the path.
 * @ingroup io
 * @details
 * !!! note
 *
 ...
 *     </ul>
 */
class API_AUI_CORE APathView: public AStringView {
private:
    using super = AStringView;

    APath ensureSlashEnding() const;
    APathView ensureNonSlashEnding() const;
...
public:
    ...
    const AStringView& string() const noexcept {
        return *this;
    }
