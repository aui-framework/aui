...
namespace aui::impl {
    template<bool cancelInDestructor>
    class AsyncHolder {
    public:
        AsyncHolder() = default;
        ~AsyncHolder() {
        ...
        template<typename T>
        AsyncHolder& operator<<(AFuture<T> future) {
            std::unique_lock lock(mSync);
        ...
        template<>
        AsyncHolder& operator<<(AFuture<void> future) {
            std::unique_lock lock(mSync);
 ...
 */
using AAsyncHolder = aui::impl::AsyncHolder<false>;
 ...
 * Guarantees that held futures will never be executed or be during execution after AAsyncHolder is destroyed.
 *
 * Unlike AAsyncHolder, cancels all futures in destructor.
 */
using AAsyncHolderCancel = aui::impl::AsyncHolder<false>;