 ...
 *
 * # Exclusive Access
 * See @ref AExclusiveAccessLockable.
 *
 */
template<typename T, typename Lockable>
class AExclusiveAccess: public aui::noncopyable {
public:
    AExclusiveAccess(T value = T()) noexcept: mValue(std::move(value)) {}
    ...
    AExclusiveAccess& operator=(const T& rhs) {
        value() = rhs;
        return *this;
    }

    AExclusiveAccess& operator=(T&& rhs) noexcept {
        value() = std::move(rhs);
        return *this;
    }
    ...
    T& value() noexcept {
#if AUI_DEBUG
        AUI_ASSERTX(mOwnerThread == AThread::current(), "AExclusiveAccess should be locked by this thread in order to get access to the underlying object");
#endif
        return mValue;
    }
