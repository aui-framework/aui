...
public:
        ...
        AWindow("COMBOBOX", 854, 500, AWindow::current())
    {
        ...
        addView(mListView = _new<AListView>(model) by(AListView, {
            setExpanding({2, 2});
        }));
    }
...
void AComboBox::onMousePressed(glm::ivec2 pos, AInput::Key button) {
    ...
    if (!mComboWindow) {
        ...
        auto w = getWindow();
        auto currentPos = w->unmapPosition(getAbsolutePosition() + w->getWindowPosition()) - w->getWindowPosition();
        ...
        mComboWindow->setAnimator(_new<ASizeAnimator>(
                glm::ivec2{mComboWindow->getWidth(), 0}) by(ASizeAnimator, {
            setDuration(0.1f);
        }));
